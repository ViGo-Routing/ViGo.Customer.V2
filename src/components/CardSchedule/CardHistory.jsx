...
import { Box, Center, HStack, IconButton, Image, Pressable, Stagger, Text, VStack, MaterialIcons, useDisclose, Icon } from 'native-base';
import { TouchableOpacity } from 'react-native-gesture-handler';
import { useNavigation } from '@react-navigation/native';
import { ArrowRightIcon, EllipsisHorizontalIcon, PencilIcon, TrashIcon, ClipboardDocumentListIcon } from "react-native-heroicons/solid";
const CardHistory = ({ element }) => {
    ...
    const formatMoney = (money) => {
        console.log(money)
        const formattedCurrency = money.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
    ...
    function handleUpdateBooking(bookingDetail) {
        navigation.navigate("UpdateRouteAndRoutine", { bookingDetailId: bookingDetail.id, position: bookingDetail });
    }
    ...
    return (
        ...
        <Box w='full' alignItems="center" p="2" _web={{
            shadow: 10,
            borderWidth: 0
        }}>

            <Box w="100%" maxW="sm" rounded="xl" p="2" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _dark={{
                ...
                <VStack alignItems="flex-end" alignSelf="flex-end">
                    <Box position="absolute" top="0" left="0"> {/* This box wraps the Stagger component */}
                        <Stagger visible={isOpen} initial={{
                            ...
                            <Pressable onPress={() => handleDetailBooking(element)} rounded="full" m={1} p={1} bg={themeColors.linear}><ClipboardDocumentListIcon size={15} color="black" /></Pressable>
                            <Pressable onPress={() => handleUpdateBooking(element)} rounded="full" m={1} p={1} bg={themeColors.linear}><PencilIcon size={15} color="black" /></Pressable>
                        </Stagger>
                    </Box>
                ...
                <VStack>
                    <HStack alignItems="center" space={4} justifyContent="flex-start">
                        ...
                        <VStack>
                            ...
                            <HStack >
                                <Text width={75} color="gray.500" bold fontSize={15}>Bắt đầu: </Text>
                                <Text numberOfLines={1} color="black" ellipsizeMode="tail" >{element.customerRoute.endStation.name}</Text>
                            </HStack>
                            <HStack >
                                <Text w={75} color="gray.500" bold fontSize={15}>Kết thúc:</Text>
                                <Text numberOfLines={1} color="black" ellipsizeMode="tail">{element.customerRoute.startStation.name}</Text>
                            </HStack>
                    ...
                    <HStack m={2} pl={20} justifyContent="flex-end" >
                        <Box backgroundColor={themeColors.linear} p="2" rounded="xl" >
                            <Text style={styles.titlePrice}>{formatMoney(element.totalPrice)}</Text>
...

                {
                    element.status !== "GOING_TO_PICKUP" ? (
                        ...
                        <Pressable onPress={() => handleTrackingDriverLocation(element)}>
                            <Text style={styles.titleButton}>Vị trí tài xế<ArrowRightIcon size={10} />  </Text>
                        </Pressable>
                    )
                }
