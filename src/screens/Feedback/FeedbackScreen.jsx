...
import { View, Avatar, Text, Box, HStack, Icon, Button, useTheme, VStack, Divider, Input, TextArea, Flex, useToast, Toast } from 'native-base';
import { StarIcon } from "react-native-heroicons/solid";
import { themeColors } from '../../assets/theme';
import Header from '../../components/Header/Header';
import SelectRouteHeader from '../../components/Header/SelectRouteHeader';
import { useNavigation } from '@react-navigation/native';
import { feedbackByBookingDetailId } from '../../service/bookingService';
...
const FeedbackScreen = ({ route }) => {
    ...
    const [selectedRating, setSelectedRating] = useState(item.rate || 1);
    ...
    const sendFeedback = async () => {
        ...
        await feedbackByBookingDetailId(item.id, requestData).then((response) => {
            if (response !== null) {
                Toast.show({
        ...
        <Flex direction="column" alignItems="center" justifyContent="center" bg="white">
            ...
            <Box p={10}>
                <VStack space={5} alignItems="center">
                    <Avatar
                        size="2xl"
                        source={{
                            uri: `${driver.avatarUrl}`, // Replace with the actual avatar URL
                        }}
                    />
                    <Text fontSize={25} fontWeight="bold">{driver.name}</Text>
                    <Text fontSize={20}>{driver.phone}</Text>
                </VStack>
            </Box>
            <Box borderColor={themeColors.primary} borderRadius="md" p={4} alignItems="center">
                ...
                <HStack space="2xl" justifyContent="center">
                    ...
                    <Text fontSize={20} fontWeight='medium'>{item.price} đ</Text>
                </HStack>
                <Divider my="2" _light={{
                    bg: "muted.800"
                }} _dark={{
