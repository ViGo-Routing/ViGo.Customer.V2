...
import { ArrowLeftIcon, RocketLaunchIcon, CalendarDaysIcon, CalendarIcon, ArrowRightIcon, ArrowsRightLeftIcon, ClockIcon } from "react-native-heroicons/solid";
import { Box, Text, Button, HStack, VStack, useToast, Divider, Alert } from "native-base";
import { SwipeablePanel } from "../../components/SwipeablePanel/Panel.tsx";
import DetailCard from "../../components/Card/DetailCard.jsx";
import { getVehicleTypeById } from "../../service/vehicleTypeService.jsx";
import { createFareCalculate, updateBookingById } from "../../service/bookingService.jsx";
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const dataResponse = {
                ...
                roundTripBeginTime: pickupBackTime === null ? pickupTime : pickupBackTime,
            };
            ...
            Alert.alert(
                ...
                [
                    ...
                    },
                    {
                        ...
                        onPress: async () => {
                            try {
                                await updateBookingById(bookingId, requestData).then((response) => {
            ...
            <Box>
                ...
                <VStack p={2}>
                    ...
                    <HStack justifyContent="space-between">
                        <Text fontSize={15} bold>Giá gốc: </Text>
                        <Text fontSize={15} >{formatMoney(fareCalculation?.originalFare)}</Text>
                    </HStack>

                    <HStack justifyContent="space-between">
                        <Text fontSize={15} bold>Phụ phí: </Text>
                        <Text fontSize={15} >{formatMoney(fareCalculation?.additionalFare)}</Text>
                    </HStack>

                    <HStack justifyContent="space-between">
                        <Text fontSize={15} bold>Tổng tiền: </Text>
            ...
            <Box>
                <View style={styles.cardInsideDateTime} m={2} >
                    <VStack justifyContent="space-evenly">
                        <HStack my={1} p={1} borderWidth={1} width="100%" borderColor="gray.200" bg="white" borderRadius={10} shadow={2} justifyContent="space-between" >
                            <HStack alignItems="center" justifyContent="center">
                                ...
                                <VStack>
                                    <Text fontSize={15} color={themeColors.primary} bold>Loại xe: </Text>
                                    <Text fontSize={15} >{vehicle?.name === null ? "" : vehicle?.name}</Text>
                                </VStack>
                            </HStack>
                            <HStack alignItems="center" justifyContent="center">
                                <Box p={2}>
                                    {routineType === null ? "" : (routineType === "ONE_WAY" ?
                                        (<ArrowRightIcon size={20} color={themeColors.primary} />) :
                                        (<ArrowsRightLeftIcon size={20} color={themeColors.primary} />))}
                                </Box>

                                <VStack>
                                    <Text fontSize={15} color={themeColors.primary} bold>Loại Chuyến: </Text>
                                    <Text fontSize={15} >{routineType === null ? "" : (routineType === "ONE_WAY" ? "Một chiều" : "Hai chiều")}</Text>
                                </VStack>
                            </HStack>
                            <HStack alignItems="center" justifyContent="center">
                                <Box p={2}>
                                ...
                                <VStack>
                                    <Text fontSize={15} color={themeColors.primary} bold>Lịch trình: </Text>
                                    <Text fontSize={15} >{routeType === null ? "" : (routineType === "WEEKLY" ? "Mỗi tuần" : "Mỗi tháng")}</Text>
                                </VStack>
                            </HStack>
                        </HStack>
                        <HStack my={1} p={1} justifyContent="space-between" >
                            <HStack borderWidth={1} width="49%" bg="white" borderColor="gray.200" borderRadius={10} shadow={2} alignItems="center" justifyContent="center">
                                <Box p={2}>
                                    <CalendarIcon size={20} color={themeColors.primary} />
                                </Box>

                                <VStack>
                                    <Text fontSize={15} color={themeColors.primary} bold>Ngày bắt đầu: </Text>
                                    <Text fontSize={15} >{routines === null ? "" : formatDateString(routines.routeRoutines[0].routineDate)}</Text>
                                </VStack>
                            </HStack>
                            <HStack borderWidth={1} width="49%" borderColor="gray.200" bg="white" borderRadius={10} shadow={2} alignItems="center" justifyContent="center">
                                <Box p={2}>
                                    <ClockIcon size={20} color={themeColors.primary} />
                                </Box>

                                <VStack>
                                    <Text fontSize={15} color={themeColors.primary} bold>Giờ đón: </Text>
                                    <Text fontSize={15} >{pickupTime === null ? "" : pickupTime}</Text>
                                </VStack>
                            </HStack>
                        </HStack>
                        ...
                        <VStack p={2}>
                            ...
                            <HStack justifyContent="space-between">
                                <Text fontSize={15} bold>Ngày trong tuần: </Text>
                                <Text fontSize={15} >{routines === null ? "" : formatDateString(routines.routeRoutines[0].routineDate)}</Text>
                            </HStack>

                            <HStack justifyContent="space-between">
                                <Text fontSize={15} bold>Ngày đặt: </Text>
                                <Text fontSize={15} >{pickupTime === null ? "" : pickupTime}</Text>
                            </HStack>

                            <HStack justifyContent="space-between">
                                <Text fontSize={15} bold>Số ngày đi: </Text>
                                <Text fontSize={15} >{routines === null ? "" : routines.routeRoutines.length}</Text>
                            </HStack>
                ...
                <Button
                    ...
                    onPress={updateBooking}
                >
                    <Text bold fontSize={20} color="white">Cập nhật</Text>
                </Button>
            </Box>
        );
    };
